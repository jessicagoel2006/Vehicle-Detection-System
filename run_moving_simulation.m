...
%RUN_MOVING_SIMULATION  Main loop with full image processing pipeline.
    ...
    while advance(scenario)
        ...
        %% Get other actor poses relative to ego
        poses = targetPoses(egoVehicle);

        %% Step 1: Generate synthetic camera frame
        frame = generate_synthetic_frame(egoPos, poses, simTime);

        %% Step 2: IP Module 1 - Canny Edge Detection (Pothole)
        [edgeImg, potholeScore, ~] = ip_edge_detection(frame);

        %% Step 3: IP Module 2 - HSV Object Detection (Cars & Peds)
        ...
        %% Step 4: IP Module 3 - Hough Lane Detection (Curve Risk)
        [laneFrame, curveScore, ~] = ip_lane_detection(frame);

        %% Step 5: IP Module 4 - Optical Flow (Motion Detection)
        [~, motionScore, ~] = ip_optical_flow(prevFrame, frame);
        prevFrame = frame;

        %% Step 6: IP Module 5 - Histogram Analysis (Visibility)
        [~, visScore, conditions] = ip_histogram_analysis(frame);
        visMultiplier = 1 + (1 - visScore) * 0.4;

        %% Step 7: Fuse all scores via weighted sigmoid
        ...
        %% Step 8: Update Dashboard every 3 frames
        if mod(frameCount, 3) == 0
            ...
            ylim(axTL,[0 1]);
            set(axTL,'Color',[0.04 0.04 0.06],'XColor','w','YColor','w');
            title(axTL, sprintf('OVERALL THREAT: %.3f [%s]', overall, threat_label(overall)), ...
    ...
    title('Full Simulation Threat Score (Image Processing)','Color','w','FontSize',12);
...
%% ====================================================================
%  LOCAL HELPERS  (ASCII only - no emoji)
%% ====================================================================
